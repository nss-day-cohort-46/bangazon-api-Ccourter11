...
class Profile(ViewSet):
    ...
    def list(self, request):
        ...
        try:
            ...
            current_user.recommendation = Recommendation.objects.filter(customer=current_user)
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    recommendation = RecommenderSerializer(many=True)

    class Meta:
        model = Customer
        fields = ('id', 'url', 'user', 'phone_number',
                  'address', 'payment_types', 'recommends','recommendation',)
        depth = 1
